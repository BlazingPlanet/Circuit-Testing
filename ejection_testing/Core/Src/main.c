...
int main(void)
{
  ...
  // Compare register set BEFORE the channel is enabled, so the very
  // first pulse out of PB0 is the armed position. Never 1500.
  __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_3, EJECT_ARMED_US);
  HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_3);

  // Countdown: LD2 blinks once per 500 ms. Hands clear.
  for (int i = 0; i < (EJECT_COUNTDOWN_MS / 500); i++)
  {
  ...
  // Fire: solid LED for the duration of the swing
  HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_SET);
